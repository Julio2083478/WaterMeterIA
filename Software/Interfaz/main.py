...
class MiApp(QMainWindow):
    ...
    def gestionar_carpetas(self):
        ...
        if archivos_inferencias:
            if not hasattr(self, "primera_ejecucion"):
                ...
                imagen_para_visualizacion = archivos_inferencias[-1]  # Esta es la imagen que usaremos para visualizacion (la última del arreglo)
                ...
                for i, fila in enumerate(datos_csv):
                    if len(fila) > 0 and fila[0] == fecha_formateada and fila[2] == hora_formateada:  # Si encuentra datos con la misma fecha y hora, se sobreescriben 
                        print("ENTRO A PONER 1111")
                        if imagen_en_proceso.startswith("1_"+ str(int(mes_inicio_mes)) + "_" + anio + "_0"):
                            # Agregar una nueva fila con medición de 0 para el primer día del mes
                            print("ENTRO A PONER CEROOOOO")
                            fecha_formateada_inicio_mes = "01/" + mes_inicio_mes + "/" + anio
                            datos_csv[i] = [fecha_formateada_inicio_mes, 0, hora_formateada] ###################
                            self.valor_dia_1_del_mes = medicion  
                            bandera = 1  
                        else:
                            datos_csv[i] = [fecha_formateada, float(medicion), hora_formateada]
                        fecha_existente = True
                        break

                # Si no hay información con la misma fecha, agregar una nueva fila al archivo CSV
                if not fecha_existente and not any(archivo.startswith("1_"+ str(int(mes_inicio_mes)) + "_" + anio + "_0") for archivo in archivos_inferencias):
                    print("ENTRO A PONER 2222")
                    datos_csv.append([fecha_formateada, float(medicion), hora_formateada])
                ...
                self.primera_ejecucion_m = medicion  # Guardar la última medición
                
                print(medicion)

                
                print("CONDICION ", ("1_"+ str(int(mes_inicio_mes)) + "_" + anio + "_0"))
                if bandera == 0 and any(archivo.startswith("1_"+ str(int(mes_inicio_mes)) + "_" + anio + "_0") for archivo in archivos_inferencias):
                    # Agregar una nueva fila con medición de 0 para el primer día del mes
                    print("ENTRO A PONER CEROOOOO")
                    fecha_formateada_inicio_mes = "01/" + mes_inicio_mes + "/" + anio
                    datos_csv.append([fecha_formateada_inicio_mes, 0, hora_formateada]) ###################
                    self.valor_dia_1_del_mes = medicion
                ...
                for imagen_en_proceso in archivos_inferencias:
                    ...
                    fecha_existente = False
                    for i, fila in enumerate(datos_csv):
                        if len(fila) > 0 and fila[0] == fecha_formateada and fila[2] == hora_formateada and not (imagen_en_proceso.startswith("1_"+ str(int(mes_inicio_mes)) + "_" + anio + "_0")):
                            print("LA IMAGEN EN PROCESO ES: ",imagen_en_proceso)
                            # Comparar con la medición anterior y guardar el valor máximo
                            medicion_actual = float(medicion)
                            medicion_anterior = self.primera_ejecucion_m
                            aux = self.valor_dia_1_del_mes
                            print("Medicion ACT:",medicion_actual,"Meidición ANTERIOR:",medicion_anterior)
                            if medicion_actual < medicion_anterior:
                                medicion = medicion_anterior
                            else:
                                self.primera_ejecucion_m = float(medicion)

                            datos_csv[i] = [fecha_formateada, int(float(medicion) - aux), hora_formateada]
                            fecha_existente = True
                            break

                    # Si no hay información con la misma fecha, agregar una nueva fila al archivo CSV
                    if not fecha_existente and not (imagen_en_proceso.startswith("1_"+ str(int(mes_inicio_mes)) + "_" + anio + "_0")):
                        print("ENTRO A 1111")
                        # Comparar con la medición anterior y guardar el valor máximo
                        medicion_actual = float(medicion)
                        medicion_anterior = self.primera_ejecucion_m
                        ...
                        datos_csv.append([fecha_formateada, int(float(medicion) - aux), hora_formateada])
                        #self.primera_ejecucion_m = float(medicion)

                    

                    # Guardar los datos actualizados en el archivo CSV
                    if not (imagen_en_proceso.startswith("1_"+ str(int(mes_inicio_mes)) + "_" + anio + "_0")):
                        print("SE ACTUALIZÓ")
                        with open(archivo_csv, mode='w', newline='') as file:
                            writer = csv.writer(file)
                            writer.writerows(datos_csv)
    ...
    def actualizarConsumoTotal(self):
        # Actualizar el DataFrame con el nuevo archivo
        self.dataframe = pd.read_csv("water_consumption_f.csv")

        # Convertir la columna 'Fecha' a tipo datetime
        self.dataframe['Fecha'] = pd.to_datetime(self.dataframe['Fecha'], dayfirst=True) #dayfirst para adaptar el formato dia/mes/año

        # Obtener el último mes registrado en el DataFrame
        ultimo_mes = self.dataframe['Fecha'].dt.month.max()

        # Filtrar los datos del DataFrame para el último mes registrado
        datos_ultimo_mes = self.dataframe[
            self.dataframe['Fecha'].dt.month == ultimo_mes
        ]

        # Verificar si hay datos para el último mes registrado
        if not datos_ultimo_mes.empty:
            # Obtener el último valor del último mes registrado
            consumo_total_mes = datos_ultimo_mes['Medición'].iloc[-1]
            consumo_total_mes = consumo_total_mes / 1000  # Se pasa a metros cúbicos para realizar el cálculo del precio
            # Actualizar el QLabel con el consumo total del mes
            self.ui.consumo_m_numero.setText("{:.2f} m³".format(consumo_total_mes))
            # PROCESO DE CALCULO DEL ACUEDUCTO
            if ultimo_mes == 3:  # marzo
                referencia_consumo = 2858
                referencia_cargo_fijo = 9586
            elif ultimo_mes == 4:  # abril
            ...
            # No hay datos para el último mes registrado
            self.ui.consumo_m_numero.setText("No hay datos para el último mes registrado")
            self.ui.precio_consumo_total.setText("No hay un precio para el último mes registrado")

        # Actualizar el estado del consumo
        self.actualizarEstadoConsumo(consumo_total_mes, ultimo_mes, total_acueducto)
